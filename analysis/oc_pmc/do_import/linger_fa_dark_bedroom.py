...
from oc_pmc.analysis.extract_theme_words import func_extract_theme_words
from oc_pmc.exclusions import exclude_linger_fa_dark_bedroom
...
def do_import_linger_fa_dark_bedroom():
    console.print("\nIMPORTING: linger-fa-dark-bedroom", style="red bold")

    success = import_data_json(
        "linger-fa-dark-bedroom",
        q_keys=q_keys,
        main_experiment_stages=main_experiment_stages,
        ratings=None,
    )
    if success:
        console.print("\nEXCLUDING linger-fa-dark-bedroom", style="red bold")
        exclude_linger_fa_dark_bedroom()

        # now extract theme words
        func_extract_theme_words({"story": "dark_bedroom", "condition": "neutralcue"})
